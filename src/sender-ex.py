...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  9 22:31:50 2019
...
def action(r):    
    ...
    global bufferFeedA
    global bufferFeedB
    global bufferMoveWait
    global bufferMoveX
    global bufferMoveY
    global bufferMoveA
    global bufferMoveB
    
    delayTime = 0.001 #delay mezi posilanim po seriove lince
    
    #vyprazdneni bufferu
    try:
        #poslani dat z bufferu na seriovou linku
        ser.write(bytes(bufferFeedX))
        sleep(delayTime)
        ser.write(bytes(bufferFeedY))
        sleep(delayTime)
        ser.write(bytes(bufferFeedA))
        sleep(delayTime)
        ser.write(bytes(bufferFeedB))
        sleep(delayTime)
        ser.write(bytes(bufferMoveWait))
        sleep(delayTime)
        ser.write(bytes(bufferMoveX))
        sleep(delayTime)
        ser.write(bytes(bufferMoveY))
        sleep(delayTime)
        ser.write(bytes(bufferMoveA))
        sleep(delayTime)
        ser.write(bytes(bufferMoveB))
        sleep(delayTime)
        #nastaveni bufferu na None aka null
        bufferFeedX = None
        bufferFeedY = None
        bufferFeedA = None
        bufferFeedB = None
...
#------------------------------interpolation--------------------------------------------
        ...
        b1,b2,b3,b4,b5,b6,b7,b8,b9 = par(1,b2,b3,b4,b5,b6,b7,b8)
        byte = bytearray([b1,b2,b3,b4,b5,b6,b7,b8,b9])
        bufferFeedY = memoryview(byte)
        #ser.write(byte)
        #sleep(0.003)
        b2,b3,b4,b5,b6,b7,b8 = i.SAP(4, 2, fa)
        b1,b2,b3,b4,b5,b6,b7,b8,b9 = par(1,b2,b3,b4,b5,b6,b7,b8)
        byte = bytearray([b1,b2,b3,b4,b5,b6,b7,b8,b9])
        bufferFeedA = memoryview(byte)
        #ser.write(byte)
        #sleep(0.003)
        b2,b3,b4,b5,b6,b7,b8 = i.SAP(4, 3, fb)
        b1,b2,b3,b4,b5,b6,b7,b8,b9 = par(1,b2,b3,b4,b5,b6,b7,b8)
        byte = bytearray([b1,b2,b3,b4,b5,b6,b7,b8,b9])
        bufferFeedB = memoryview(byte)
        #ser.write(byte)
        #sleep(0.003)
#---------------------------interpolation-end-------------------------------------------     
        ...
        if nx == mx:
            ...
            bufferMoveWait = memoryview(bytearray([1,138,0,0,0,0,0,1,140]))
            #sleep(0.001)
            bufferMoveX = memoryview(x)
            #ser.write(x)
            #sleep(0.001)
            bufferMoveY = memoryview(y)
            #ser.write(y)
            #sleep(0.001)
            bufferMoveA = memoryview(a)
            #ser.write(a)
            #sleep(0.001)
            bufferMoveB = memoryview(b)
            #ser.write(b)
            print("first")
            pass
        elif ny == mx:
            #ser.write(bytearray([1,138,0,0,0,0,0,2,141]))
            bufferMoveWait = memoryview(bytearray([1,138,0,0,0,0,0,2,141]))
            #sleep(0.001)
            bufferMoveX = memoryview(x)
            #ser.write(x)
            #sleep(0.001)
            ...
            bufferMoveWait = memoryview(bytearray([1,138,0,0,0,0,0,4,143]))
            #sleep(0.001)
            bufferMoveX = memoryview(x)
            #ser.write(x)
            #sleep(0.001)
            ...
            bufferMoveWait = memoryview(bytearray([1,138,0,0,0,0,0,8,147]))
            #sleep(0.001)
            bufferMoveX = memoryview(x)
            #ser.write(x)
            #sleep(0.001)
...
with open(file, "rb") as t:
    ...
    for line in t:
        ...
        if (aLast.find("G0") != -1) or (aLast.find("G1") != -1):
            action(line)
            while True:
                r = ser.read(9)
                print(r)
            ...
            while True:
                ...
                if (Xh == 1) and (Yh == 1) and (Ah == 1) and (Bh == 1):
                    ...
                    print("home finished")
                    break
        elif (aLast.find("none") != -1 or aLast.find("G90") != -1 or aLast.find("G91") != -1 or aLast.find("M104") != -1 or aLast.find("G28") != -1):
            action(line)
            #break
        else:
            action(line)
